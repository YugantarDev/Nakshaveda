...
import astrologerData from '../data/astrologers'
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  introLeft.querySelector('.subheading').textContent =
    siteContent.intro.subheading
 ...
 `
  ...
  window.addEventListener('scroll', () => {
    ...
    const heroScaleValue = Math.max(
      0.5,
      1 - scrollPosition / (heroHeight * 0.7)
    )
    const heroOpacityValue = Math.max(
      0,
      1 - scrollPosition / (heroHeight * 0.7)
    )
  ...
  // Modal elements
  const modal = document.getElementById('astrologerModal')
  const modalDetails = document.getElementById('modal-details')
  const closeBtn = document.querySelector('.close-button')

  // Function to open the modal and populate with data
  const openModal = astro => {
    // Clear previous content
    modalDetails.innerHTML = ''

    // Create and append the card content to the modal
    const modalCard = createCard(astro)
    modalDetails.appendChild(modalCard)

    // Make the modal visible and trigger animation
    modal.style.display = 'flex'
    setTimeout(() => {
      modal.classList.add('show')
    }, 10) // Small delay to allow CSS display change
  }
 ...
 `
  ...
  const updateCarousel = (isDirectClick = false) => {
    ...
    astroTrack.style.transition = isDirectClick
      ? 'none'
